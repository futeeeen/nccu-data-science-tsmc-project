...
TRAINING_DATA_TERMS = {
    ...
    'expectancy': {
        'aliases': ['expected value', 'validation_expectancy', 'expectancy per trade'],
        'definition': 'The average expected return per trade based on win rate, average win, loss rate, and average loss.',
        'usage': 'Used in the Triple Barrier validation objective so threshold search prefers strategies with positive statistical edge.',
        'interpretation': 'Positive expectancy means the average trade is favorable; negative expectancy means the strategy may lose even if some trades win.',
    },
    'profit_factor': {
        'aliases': ['validation_profit_factor', 'gross profit divided by gross loss'],
        'definition': 'Gross winning trade return divided by absolute gross losing trade return.',
        'usage': 'Used to judge whether total wins meaningfully exceed total losses.',
        'interpretation': 'Values above 1 mean wins exceed losses; values below 1 mean losses dominate.',
    },
    'win_rate': {
        'aliases': ['validation_win_rate', 'winning percentage'],
        'definition': 'The percentage of trades with positive return.',
        'usage': 'Shown with avg_win and avg_loss because high win rate alone does not guarantee a profitable strategy.',
        'interpretation': 'A lower win rate can still work if average wins are much larger than average losses.',
    },
    'avg_win': {
        'aliases': ['average win', 'mean winning trade'],
        'definition': 'The average return of winning trades.',
        'usage': 'Part of expectancy and payoff ratio calculations.',
        'interpretation': 'Larger average wins improve statistical edge if losses are controlled.',
    },
    'avg_loss': {
        'aliases': ['average loss', 'mean losing trade'],
        'definition': 'The average return of losing trades.',
        'usage': 'Part of expectancy and payoff ratio calculations.',
        'interpretation': 'More negative average losses can destroy a strategy even when win rate looks high.',
    },
    'validation_objective': {
        'aliases': ['threshold objective', 'risk adjusted objective'],
        'definition': 'The score used to select validation thresholds, combining expectancy, Sharpe reward, drawdown penalty, and turnover penalty.',
        'usage': 'Replaces selecting thresholds only by validation return.',
        'interpretation': 'Higher objective means a threshold has better statistical edge after risk and overtrading penalties.',
    },
    'entry_filter_ok': {
        'aliases': ['anti chasing filter', 'entry discipline filter'],
        'definition': 'Whether the anti-chasing entry filters allow a buy signal on that row.',
        'usage': 'The strategy only enters when final_score passes the threshold and entry_filter_ok is true.',
        'interpretation': 'False means the model score may be high, but the setup looks too extended or crowded to enter.',
    },
    'entry_filter_overextended': {
        'aliases': ['overextended filter', 'overheated price filter'],
        'definition': 'A filter that blocks entries when RSI or price distance from the 20-day moving average is too high.',
        'usage': 'Designed to reduce late entries after a sharp price run-up.',
        'interpretation': 'True means the strategy treats the price as potentially overextended.',
    },
    'entry_filter_crowded': {
        'aliases': ['crowded filter', 'volume volatility crowding filter'],
        'definition': 'A filter that blocks entries when ATR percentile or volume-change percentile is too high.',
        'usage': 'Designed to avoid entering during crowded, emotional, or late-stage moves.',
        'interpretation': 'True means current volatility or volume behavior is unusually hot.',
    },
    'entry_blocked_count': {
        'aliases': ['blocked entries', 'validation_entry_blocked_count'],
        'definition': 'The number of buy-threshold signals blocked by anti-chasing entry filters.',
        'usage': 'Helps inspect how often the discipline layer rejected high-score but risky entries.',
        'interpretation': 'Higher values mean the model wanted to enter more often, but filters saw crowded or overextended conditions.',
    },
    'pred_prob_up': {
    ...
    'trend_score': {
        'aliases': ['trend model score', 'trend filter'],
        'definition': 'A 0-100 rule-based score that summarizes whether the longer-term trend is supportive.',
        'usage': 'Used before entry so the strategy separates broad trend judgment from the exact entry timing decision.',
        'interpretation': 'Higher values mean price, moving averages, slopes, and multi-week returns are more consistently bullish.',
    },
    'entry_timing_score': {
        'aliases': ['entry score', 'timing score', 'entry timing filter'],
        'definition': 'A 0-100 score that checks whether the current price is a patient entry setup rather than a chase.',
        'usage': 'Combines pullback quality, RSI balance, and volatility contraction information before allowing a trade.',
        'interpretation': 'Higher values mean the setup is closer to an orderly entry zone; very low values warn against chasing.',
    },
    'entry_filter_trend_timing': {
        'aliases': ['trend timing filter ok'],
        'definition': 'A boolean flag showing whether both trend_score and entry_timing_score passed their thresholds.',
        'usage': 'If false, a high final_score signal can still be blocked before entry.',
        'interpretation': 'True means the trend and timing layer accepts the setup; false means the strategy waits.',
    },
    'raw_position': {
        'aliases': ['position before derisk', 'pre derisk position'],
        'definition': 'The position size produced by the entry and risk-per-trade rules before drawdown de-risking.',
        'usage': 'Useful for comparing intended exposure with the final position after drawdown controls.',
        'interpretation': 'A value below 1 means the model is using partial exposure rather than all-in positioning.',
    },
    'drawdown_derisk_multiplier': {
        'aliases': ['derisk multiplier', 'drawdown multiplier'],
        'definition': 'The multiplier applied to raw_position after the strategy equity curve enters a configured drawdown.',
        'usage': 'Reduces exposure during weaker periods to enforce risk discipline.',
        'interpretation': '1 means no reduction; 0.5 means the current raw position is cut in half.',
    },
    'risk_per_trade_pct': {
        'aliases': ['risk per trade', 'position sizing risk'],
        'definition': 'The fraction of capital the strategy is willing to risk if the stop-loss barrier is hit.',
        'usage': 'Position size is estimated as risk_per_trade divided by stop_loss_pct, capped by max_position_size.',
        'interpretation': 'Smaller values make the strategy more conservative and reduce drawdown sensitivity.',
    },
    'max_position_size': {
        'aliases': ['maximum position size', 'position cap'],
        'definition': 'The maximum allowed exposure for a single position.',
        'usage': 'Caps risk-based position sizing so low ATR stop distances do not create oversized positions.',
        'interpretation': '100% means the strategy can be fully invested; lower values force partial exposure.',
    },
    'derisk_drawdown_pct': {
        'aliases': ['drawdown derisk trigger', 'derisk trigger'],
        'definition': 'The drawdown level at which the strategy starts reducing exposure.',
        'usage': 'Used with drawdown_derisk_multiplier to cut position size after the equity curve weakens.',
        'interpretation': 'Lower trigger values react earlier; higher trigger values tolerate deeper drawdowns.',
    },
    'purged_embargo_split': {
        'aliases': ['purged validation', 'embargo split', 'enable_purged_embargo'],
        'definition': 'A time-series validation method that removes samples whose Triple Barrier event windows overlap across train, validation, and test boundaries.',
        'usage': 'Reduces look-ahead leakage caused by overlapping event labels in financial machine learning.',
        'interpretation': 'When enabled, usable rows may decrease, but validation results are methodologically cleaner.',
    },
    'embargo_days': {
        'aliases': ['embargo period'],
        'definition': 'The number of calendar days removed around split boundaries after purging overlapping Triple Barrier events.',
        'usage': 'Adds a buffer between train, validation, and test windows.',
        'interpretation': 'Larger values reduce leakage risk but leave fewer training and validation rows.',
    },
    'walk_forward_folds': {
        'aliases': ['walk-forward validation', 'validation_fold_count'],
        'definition': 'The number of chronological validation folds used during threshold search.',
        'usage': 'Instead of selecting a threshold from one validation window, the app can average the objective across several consecutive validation folds.',
        'interpretation': 'More folds make threshold selection less dependent on one lucky period, but each fold has fewer rows.',
    },
}
